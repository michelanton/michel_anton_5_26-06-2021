...
function getArticle() {
  ...
  fetch(UrlApi + urlProduit) // appel connection server
    .then(function (
      response // reponse eu format JSON "string"
    ) {
      return response.json(); // transforme JSON en JS
    })
    .catch(
      (
        error //si erreur de connection au server
      ) => {
    ...
    .then((resultatAPI) => {
      ...
      envoyerPanier.addEventListener("click", (e) => {
        // ecouter le click (event) sur bouton panier
        ...
        const popupConfirmationPanier = () =>
          // ccomportement de la fenetre confirm
          {
            if (
              window.confirm(
                "vous avez rajouté :" +
